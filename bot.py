...
# ================= CONFIG =================
BOT_TOKEN = "6367532329:AAFUobZTDtBrWWfjXanXHny9mBRN0eHyAGs"   # token của bạn
...
POLL_INTERVAL = 15   # giây (khoảng thời gian bot auto đọc kết quả)
# ==========================================
...
def update_model(model: Dict[str, Any], predict: str, actual: str):
    if predict == actual:
        ...
        for k in model["weights"]:
            model["weights"][k] = round(min(3.0, model["weights"][k] * 1.03), 2)
    else:
        model["stats"]["lose"] += 1
        for k in model["weights"]:
            model["weights"][k] = round(max(0.2, model["weights"][k] * 0.97), 2)
